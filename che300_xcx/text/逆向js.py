...
class HuanYuan_Photo:
    ...
    def get_photo(self):
        ...
        p1_img = './dx_img/' + json_data['p1'].split('/')[-1].split('.')[0] + '.jpg'
        ...
        p1_img = './dx_img/' + json_data['p2'].split('/')[-1].split('.')[0] + '.jpg'
        with open(p1_img, 'wb') as f:
            f.write(response.content)
        f.close()
        ...
        p3_img = './dx_img/' + json_data['p3'].split('/')[-1].split('.')[0] + '.jpg'
    ...
    def get_yuantu(self, photo_name):
        ...
        # 切割
        img = Image.open('./dx_img/' + photo_name + '.jpg')
        for i in range(32):
            a = 12 * i  # 图片距离左边的大小
            b = 0  # 图片距离上边的大小
        ...
        target.save(f'./dx_img/{photo_name}.jpg')


if __name__ == '__main__':
    huanyuan = HuanYuan_Photo()
    p1_name, p3_name = huanyuan.get_photo()
    for i in [p1_name, p3_name]:
        huanyuan.get_yuantu(i)
