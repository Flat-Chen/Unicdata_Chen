...
class AutohomeLuntanVideoSpider(scrapy.Spider):
    name = 'autohome_luntan_video'
    ...
    def __init__(self, **kwargs):
        super(AutohomeLuntanVideoSpider, self).__init__(**kwargs)
        self.num = 1
        self.counts = 0
    ...
    custom_debug_settings = {
        ...
        'MYSQL_PORT': 3306,
        'MYSQL_DB': "saicnqms",
        'MYSQL_TABLE': "luntan_autohome_lostvideo",
    ...
    def start_requests(self):
        ...
        for lost_url in lost_urls_list:
            ...
            meta = {'brand': lost_url['brand'], 'factory': lost_url['factory'], 'url': url, '_id': lost_url['_id'],
                    "posted_time": lost_url['posted_time']}
    ...
    def parse(self, response):
        ...
        if 'safety' in response.url:
            ...
            item["user_name"] = response.xpath("//a[@class='user-name']/text()").extract_first()
            item["posted_time"] = posted_time
            item["user_car"] = response.xpath(
                "//div[@class='topinfo']/a[@class='seriesname']/text()").extract_first().strip("论坛")
            ...
            item["reply_num"] = response.xpath("//text[@id='replyCount']/text()").extract_first()
            ...
            item["content_num"] = response.xpath("////span[@id='followCount']/text()").extract_first()
            # print(item)
            if item["content"] == "":
                print('------------------------------------------内容为空，pass-----------------------------------')
