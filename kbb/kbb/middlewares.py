...
class KBBProxyMiddleware(object):
    def __init__(self):
        self.count = 0
    ...
    def process_request(self, request, spider):
        if spider.name in ['kbb_new', 'che300_price_daily']:
            # if self.count < 2:
            #     request.meta['proxy'] = self.proxy
            #     self.count += 1
            # else:
...
class KBBUserAgentMiddleware(UserAgentMiddleware):
    def process_request(self, request, spider):
...
class KbbSpiderMiddleware(object):
...
class KbbDownloaderMiddleware(object):
    # Not all methods need to be defined. If a method is not defined,
