...
class GonggaoProxyMiddleware(object):
    def __init__(self):
        self.count = 0
    ...
    def process_request(self, request, spider):
        if spider.name in ['diyidiandong', 'shangche']:
            # if self.count < 2:
            #     request.meta['proxy'] = self.proxy
            #     self.count += 1
            # else:
...
class GonggaoUserAgentMiddleware(UserAgentMiddleware):
    def process_request(self, request, spider):
...
class GonggaoSpiderMiddleware(object):
...
class GonggaoDownloaderMiddleware(object):
    # Not all methods need to be defined. If a method is not defined,
